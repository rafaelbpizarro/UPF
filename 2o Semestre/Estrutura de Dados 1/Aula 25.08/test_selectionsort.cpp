/*
Codificar, dentro da biblioteca ordenacao.cpp, o template de função:

template<typename T>
void selectionsort(T vet[], int n) {
}

Ele recebe um vetor qualquer de um tipo genérico (T) e o seu tamanho (n).
A função deve ordenar (classificar) o vetor em ordem crescente, usando o método de ordenação por Seleção Direta.
